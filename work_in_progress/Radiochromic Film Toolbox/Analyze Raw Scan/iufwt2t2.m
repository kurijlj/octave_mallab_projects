function f = iufwt2t2(ca, ch, cv, cd, par, varargin)
...
% AUTHOR: Zdenek Prusa
    ...
    complainif_notenoughargs(nargin, 5, 'IUFWT');

    if(isstruct(par) && isfield(par, 'fname'))
       complainif_toomanyargs(nargin, 5, 'IUFWT');

       if ~strcmpi(par.fname, 'ufwt2')
          error( ...
              [ ...
                  '%s: Wrong func name in info struct. ', ...
       ...
       complainif_notenoughargs(nargin, 6, 'IUFWT2');
    ...
    %%  Run computation
    f = comp_iufwt2(ca, ch, cv, cd, w.g, w.a, J, scaling);
