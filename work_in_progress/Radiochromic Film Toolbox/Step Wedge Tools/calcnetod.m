function mnod = calcnetod(msig, mbkg, varargin)
%% -----------------------------------------------------------------------------
%%
%% Function 'calcnetod':
%%
...
% -----------------------------------------------------------------------------
    ...
    endif;  % 0 ~= numel(pos) && 1 ~= numel(pos)

    % Validate optional arguments ---------------------------------------------

    % If zero-light argument is not supplied set it to the default value. We use
    % copy of the data sample from the control piece measurement as a template
    % for the zero-light measurement
    dsbkg = mbkg.at(1);
    mzrl = Measurement( ...
        DataSample( ...
            dsbkg.position, ...
    ...
    while(msig.numel() >= idx)
        ...
        nodstd = ((sqrt(dssig.n) * rcorr) .* ( ...
                (dsbkg.stderr(3) + dszrl.stderr(3))./(bcorr) ...
