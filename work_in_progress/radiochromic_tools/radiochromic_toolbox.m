...
function rct_update_gui_test(obj)
    h = guidata(obj);
    ...
    switch(gcbo)
        case {h.level_selector}
            ...
            % Re-calculate selection frame size and position and replot
            % histogram.
        ...
        case {h.window_selector}
            ...
            % level_value = (level_max - level_min) / 2;

            set(h.level_selector, 'min', level_min);
            set(h.level_selector, 'max', level_max);
            if(level_min > level_value)
                level_value = level_min;
                % set(h.level_selector, 'value', level_value);

            elseif(level_max < level_value)
            ...
            set(h.window_label, 'string', sprintf("Window: %d", window_value));

            if(h.value_range(1, 2) == window_value)
...
function rct_gui_test(img, nbins)
    ...
    level_min = value_range(1, 1) + (window_value / 2);
    level_max = value_range(1, 2) - (window_value / 2);
    % level_step = (level_max - level_min) / 100;
